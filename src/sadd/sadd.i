...
# 1 "./sadd.inc" 1 
	PARAMETER	(KAPPA=1024)
	PARAMETER	(MAXKAPPA=2048)
	PARAMETER	(MAXREGIS=3)
	
	CHARACTER*80	FNINP,FNADD,FNOUT
	CHARACTER*1	FORMI,FORMA,FORMO
	CHARACTER*80	SERV

	COMMON /SDATI/	RB1(0:MAXKAPPA*1024-1),
     1		RB2(0:MAXKAPPA*1024-1),
     1		RB3(0:MAXKAPPA*1024-1)
	COMMON /STORE/	IREG,RBSTOR(0:MAXKAPPA*1024-1,0:MAXREGIS)
	COMMON /SFILES/	FNINP,FNADD,FNOUT,FORMI,FORMA,FORMO
	COMMON /SADSVC/	KLI,KLA,KLO,ISUC,ICOUNT,NSVC,ISVC
	COMMON /SADSV1/	SERV(20)
# 4 "sadd.F" 2 
...
# 877 "sadd.F" 2 
	...
	CALL INP_SHOWBUFF(IL)
	IF(IL.ne.0) THEN
	   CALL INP_str('Filename to cycle',ch80)
	   if(ch80(1:1).eq.'+') then
	     tipo=1
...
# 998 "sadd.F" 2 
	...
	IF(KLO.GT.99) LNKAP=3
	LFNAM=LENGTHC(FNOUT)
	WRITE(10,15) FNOUT(1:lfnam),FORMO,KLO
	WRITE(6,15)  FNOUT(1:lfnam),FORMO,KLO
...
# 1077 "sadd.F" 2 
	...
	IIC=INP_STR('DCL command',DCLCOM)
	IF(IIC.LT.0) GOTO 99
	IL=LENGTHC(DCLCOM)
	IF(IL.GT.0) THEN
	  ISTATUS=Lib$Spawn(DCLCOM)
	ELSE
	  ISTATUS=Lib$Spawn()
	ENDIF
	IF (.NOT.ISTATUS) THEN
